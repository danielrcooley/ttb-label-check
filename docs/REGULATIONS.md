...
|---|---|---|
| Anchor in capitals and bold | "The first two words ... 'GOVERNMENT WARNING,' shall appear in capital letters and in bold type." | Capitals: yes (from the OCR read). Bold: measured from the pixels as the heading's stroke weight against the rest of its own line, split at the word gap where the heading ends (D-044, D-045); a heading 1.20 times heavier or more is Match, the same weight is Needs review, a small difference is inconclusive, small print or a boundary not found in the print is "Not checked" with the reason |
| Remainder not bold | "The remainder of the warning statement may not appear in bold type." | The same measurement, relatively: a body as heavy as the heading is Needs review (the whole statement may be bold); a body lighter than the heading is "Not checked", because the measurement cannot tell a regular body under a bold heading from a bold body under a heavier one. Never a failure |
