...
| # | Finding | Disposition |
|---|---|---|
| 1.1 | ASCII masking could turn genuinely accented warning text into the required spelling and pass it as exact | **Accepted as a documented consequence, not changed.** The alphabet restriction is the engineer's decision (D-032) and stays on by default. The false-pass direction is now stated in LIMITS.md item 11, README (error analysis) and the module docstring. The switch `TTB_OCR_ASCII_ALPHABET` is documented. |
