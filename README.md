...
|---|---|---|
| "If we can't get results back in about **5 seconds**, nobody's going to use it." | OCR runs in-process on a small neural model; the images of one application are read in parallel; every result prints its own timing. | Local: front+back application median 2.5 s, p95 3.1 s on two workers. Deployed (Azure, 2 vCPU): one person, front+back, median 2.7 s, p95 2.8 s over 20 runs; a front label alone, which triggers the sideways re-read, median 3.2 s, p95 3.3 s; two people submitting at once, p95 5.0 s (see [Measured](#measured)) |
| "something **my mother could figure out**" | One screen, two numbered steps, one big button, three one-click samples, U.S. Web Design System, statuses as icon + word + color, keyboard and screen-reader friendly, pressed buttons marked in text as well as color so Windows contrast themes show them. | Observed usability test: _USABILITY_RESULT_ |
